...
function safeParseInvoice(response) {
    return {
        bill_to: typeof response.bill_to === "string" ? response.bill_to.trim() : null,
        subtotal: cleanNumber(response.subtotal),
        total: cleanNumber(response.total),
    };
}
export default async function groqTest(img) {
    await delay(5000);
    const chatCompletion = await groq.chat.completions.create({
        messages: [
            {
                role: "system",
                content: "You are an invoice parser AI. Always respond ONLY with a JSON object having the following exact keys: bill_to, subtotal, total. Use null for any missing values. Do not include extra explanation or formatting.",
            },
            {
                role: "user",
                content: [
